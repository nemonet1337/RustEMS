...
| フレーム（VER/FLAGS/SEQ/LEN/PAYLOAD/CRC16, 断片化フラグ） | `device-api/src/frame.rs` | ✅ 実装・テスト済 |
| メッセージヘッダ・オペコード・エラー/値型の列挙 | `device-api/src/message.rs` | ✅ 実装・テスト済 |
| 断片化の再結合ヘルパ | `device-api` | ⏳ 未着手 |
| CBOR ボディコーデック | `device-api` | ⏳ 未着手 |
| パラメータカタログ生成（`codegen` 転用） | `codegen` | ⏳ 未着手 |
| デバイス側ハンドラ | `engine-core/comms` | ⏳ 未着手 |
| USB / Bluetooth トランスポート | `stm32` / host | ⏳ 未着手 |
| ホスト側クライアント | `client` / `cli` | ⏳ 未着手 |
