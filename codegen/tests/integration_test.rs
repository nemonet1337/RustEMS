...
const CONFIG_TXT_PATH: &str = concat!(
...
fn test_parse_real_config() {
    ...
    let content = fs::read_to_string(config_path).expect("Failed to read rusefi_config.txt");
        ...
        Ok(lines) => {
            ...
            let significant = lines
                ...
                .filter(|l| {
                    !matches!(
                        l,
                        rusefi_codegen::parser::ConfigLine::Empty
                            | rusefi_codegen::parser::ConfigLine::Comment(_)
                    )
                })
                .count();
...
"#;
    ...
    assert!(defines
        .iter()
        .any(|(k, _)| k.to_lowercase() == "test_value"));
...
fn test_compare_with_java_output() {
    ...
    assert_eq!(
        differences, 0,
        "Found {} differences between Java and Rust output",
        differences
    );
}
