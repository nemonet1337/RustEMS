...
//! on the 12-cylinder-capable Proteus board. Injectors idle low and are driven
//! high while open.
//!
//! Pin assignment (PF0..PF11) is nominal and must be matched to the production
//! schematic; it avoids the ADC (PA/PC), trigger (PA5/PA8), CAN and ignition
//! (PE4..PE15) pins used elsewhere on the board.
...
/// Maximum injector channels for Proteus (12-cylinder capable).
pub const INJ_COUNT: usize = 12;

/// Proteus Injector Output driver for up to 12 cylinders.
...
impl Stm32InjectorOutput {
    ...
    fn set_injector(&mut self, cylinder: u8, state: bool) {
        ...
        if let Some(inj) = self.injectors.get_mut(cylinder as usize) {
            ...
            defmt::warn!("Invalid cylinder {} for Proteus injector", cylinder);
        }
    }
}
