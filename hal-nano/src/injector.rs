...
//!
//! Controls 8 fuel injectors (High-Z).
//! PB9=INJ1, PB8=INJ2, PD15=INJ3, PD14=INJ4,
//! PD13=INJ5, PD12=INJ6, PD11=INJ7, PD10=INJ8

use rusefi_core::hal::InjectorOutput;
use embassy_stm32::{Peri, gpio::{Level, Output, Speed}};
use embassy_stm32::peripherals::{PB8, PB9, PD10, PD11, PD12, PD13, PD14, PD15};

/// Maximum number of injector channels on the Nano board.
pub const INJ_COUNT: usize = 8;

/// Nano Injector Output driver for 8 channels.
...
impl Stm32InjectorOutput {
    /// Create a new 8-channel injector output driver.
    pub fn new(
        pb9:  Peri<'static, PB9>,
        pb8:  Peri<'static, PB8>,
        ...
        Self {
            ...
            inj7: Output::new(pd11, Level::Low, Speed::High),
            inj8: Output::new(pd10, Level::Low, Speed::High),
        }
    }
    ...
    fn set_injector(&mut self, cylinder: u8, state: bool) {
        ...
        match cylinder {
            ...
            6 => self.inj7.set_level(level),
            7 => self.inj8.set_level(level),
            _ => defmt::warn!("Invalid cylinder {} for Nano injector (max 8)", cylinder),
        }
    }
}
