            ...
            Ok(state) => {
                    ...
                    && matches!(state.sync, SyncState::CrankSynced | SyncState::FullSync)
                {
                    if let Some(rpm) = state.rpm {
                        ...
                        #[cfg(feature = "fuel-fi")]
                        let (dfco_active, cl_correction, ltft_correction, accel_mult) = {
                            let da = dfco.update(rpm, map_kpa, tps_pct, ts);
                            if da {
                                closed_loop.trigger_pause();
                            }
                            let cl = closed_loop.update(rpm, clt_c, lambda_meas, dt_s);
                            ltft.update(&sensors, 1.0);
                            let ltft_corr = ltft.get_trim(rpm, map_kpa / 101.325 * 100.0);
                            let am = accel.update(tps_pct, ts);
                            (da, cl, ltft_corr, am)
                        };

                        // ── Idle / boost per-cycle update ──────────────────
                        let iac2 = idle.update(
                            rpm,
                            clt_c,
                            tps_pct,
                            rpm < cfg.cranking_rpm,
                            dt_s * 1000.0,
                            false,
                        );
                        ...
                        for (i, &cyl) in cfg.firing_order.iter().enumerate() {
                            ...
                            if !dfco_active {
                                let airmass =
                                    estimate_airmass_g(map_kpa, cfg.displacement_cc_per_cyl, 0.85);
